...
class SendPoMailToAdmin implements ShouldQueue
{
    ...
    public function handle(): void
    {
        $accountsUsers = User::where('role_id', function ($q) {
            $q->select('id')->from('roles')->where('name', 'Administrator');
        })
            ->whereNotNull('email')
            ->active()
            ->get();
        ...
        if ($accountsUsers->isEmpty()) {
            Log::info('No Administrator users found — PO mail skipped', ['po' => $this->invoice->po_number]);
            return;
        }
    ...
    {
        Log::error('SendPoMailToAdmin failed after all retries', [
