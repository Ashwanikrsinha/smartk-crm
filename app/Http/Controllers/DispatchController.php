...
class DispatchController extends Controller
{
    ...
    public function show(Dispatch $dispatch)
    {
        $invoice = Invoice::with([
            'customer',
            'invoiceItems.product.category',
            'dispatches' => fn($q) => $q->orderByDesc('dispatch_date')->orderByDesc('id'),
            'dispatches.items.product.category',
            'dispatches.dispatchedBy',
            'dispatches.editedBy',
        ])->findOrFail($dispatch->invoice_id);

        return view('dispatches.show', [
            'invoice'    => $invoice,
            'dispatches' => $invoice->dispatches,
            'canEdit'    => $this->canEditDispatches(),
        ]);
    } 

    public function update(Request $request, Dispatch $dispatch)
    {
        abort_unless($this->canEditDispatches(), 403, 'You are not authorized to edit dispatches.');

        if ($dispatch->isEdited()) {
            return back()->with('error', 'This dispatch has already been edited once and cannot be edited again.');
        }
        ...
        return back()->with('success', "Dispatch {$dispatch->dispatch_number} updated.");
    }

    private function canEditDispatches(): bool
    {
        $roleName = auth()->user()->role?->name;

        return in_array($roleName, ['Administrator', 'Accounts'], true);
    } 
    public function poSummary(Invoice $invoice)
    {
        $this->authorize('view', $invoice);
