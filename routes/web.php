...
// ════════════════════════════════════════════════════════════════════════
// ALL AUTHENTICATED ROUTES — single group, no duplicates
// ════════════════════════════════════════════════════════════════════════
Route::middleware('auth')->group(function () {
    ...
    // Lead Sources (new)
    Route::resource('lead-sources', Controllers\LeadSourceController::class);
    ...
    // Visit attachments — BEFORE resource to avoid wildcard conflict
    ...
    // CRITICAL: All custom GET routes MUST come before Route::resource()
    // otherwise {invoice} wildcard swallows them
    // ════════════════════════════════════════════════════════════════════
    ...
    // Approval actions (PATCH — before resource)
    ...
    Route::get('invoices/{invoice}/document/download',[Controllers\InvoiceController::class, 'downloadDocument'])->name('invoices.document.download');

    // PO Document — regenerate stored DOCX (SM / Admin)
    Route::patch('invoices/{invoice}/document/regenerate',[Controllers\InvoiceController::class, 'regenerateDocument'])->name('invoices.document.regenerate');

    // PO Mail — resend to specific recipient or all
    Route::post('invoices/{invoice}/mail/resend',[Controllers\InvoiceController::class, 'resendMail'])->name('invoices.mail.resend');
    ...
    Route::get(
        'invoices/school-from-po/{invoice}',
        [Controllers\BillController::class, 'schoolFromPo']
    )
        ->name('bills.school-from-po');


    // ════════════════════════════════════════════════════════════════════
    // COLLECTIONS (Accounts Team payment updates)
    // CRITICAL: POST bulk route BEFORE resource()
    // ════════════════════════════════════════════════════════════════════
    Route::post('collections/bulk',               [Controllers\CollectionController::class, 'bulkStore'])->name('collections.bulk-store');
    Route::get('collections/invoice/{invoice}',   [Controllers\CollectionController::class, 'forInvoice'])->name('collections.for-invoice');

    // Resource — only needed actions
    ...
    // ════════════════════════════════════════════════════════════════════
    // REPORTS
    // IMPORTANT: Route::view('reports',...) at bottom of old file has been
    // REMOVED. Only this controller route exists now.
    // ════════════════════════════════════════════════════════════════════
    Route::get('reports/export',  [Controllers\ReportsController::class, 'export'])->name('reports.export');
...
Route::middleware('auth')->name('tally.')->prefix('tally')->group(function () {
    ...
    Route::get(
        'sales/{sale}',
        fn(App\Models\Tally\Sale $sale) =>
        view('tally.sales.show', compact('sale'))
    )->name('sales.show');

    Route::get(
        'purchases/{purchase}',
        fn(App\Models\Tally\Purchase $purchase) =>
        view('tally.purchases.show', compact('purchase'))
    )->name('purchases.show');

    Route::get(
        'receipts/{receipt}',
        fn(App\Models\Tally\Receipt $receipt) =>
        view('tally.receipts.show', compact('receipt'))
    )->name('receipts.show');

    Route::get(
        'payments/{payment}',
        fn(App\Models\Tally\Payment $payment) =>
        view('tally.payments.show', compact('payment'))
    )->name('payments.show');
});
