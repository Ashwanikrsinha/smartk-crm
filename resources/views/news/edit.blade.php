...
<form action="{{ route('news.update', ['news' => $news]) }}" method="POST" class="p-3 shadow-sm rounded bg-white">
    @method('PUT')
    @include('news.form', ['mode' => 'edit'])
</form>
    ...
    <div class="card-body">
        <div class="row">
            @foreach ($news->images as $image)
                ...
                <form action="{{ route('news.images.destroy', ['image' => $image ]) }}" method="POST"
