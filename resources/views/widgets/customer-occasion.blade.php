<div class="card border-0 shadow-sm mb-4">
    <header class="card-header bg-white py-3 d-flex align-items-center justify-content-between">
      ...
      <span class="badge bg-danger rounded-pill">{{ $contact_occassions->count() }}</span>
    </header>
    <div class="card-body px-0 pt-0 table-responsive">
       @if ($contact_occassions->count() > 0)
       <table class="table align-middle" style="min-width: 30rem;">
          ...
          <tbody>
             @foreach ($contact_occassions as $contact)
                 <tr>
                    <td class="ps-3">
                       <p class="mb-0">{{ $contact->name }} &bull; {{ $contact->designation }}</p>
                       <small class="text-muted">({{ $contact->name }})</small>
                   </td>
                    <td>
                      <p class="mb-1">
                         @if(isset($contact->birth_date) && $contact->birth_date->format('m-d') == date('m-d'))  
                         {{ $contact->birth_date->format('d M, Y') }}
                         <span class="badge alert-primary ms-1">Birthday</span>
                         @endif
                      </p>

                       @if(isset($contact->marriage_date) && $contact->marriage_date->format('m-d') == date('m-d'))  
                       {{ $contact->marriage_date->format('d M , Y') }}
