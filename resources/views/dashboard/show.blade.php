...
<div class="alert bg-white shadow-sm  d-flex align-items-center justify-content-between py-2 mb-4">
   ...
   <form action="{{ route('storage.link.create') }}" method="GET" class="d-inline-block">
...
@if ($notification_count > 0)  
   <div class="alert bg-white shadow-sm mb-3 d-flex align-items-center justify-content-between">
      <span>
         ...
         You has <span class="badge bg-danger rounded-pill mx-1">{{ $notification_count }}</span> new notifications
      </span>
...
{{-- birthday greetings --}}
@if (isset(auth()->user()->birth_date) && auth()->user()->birth_date->format('m-d') == date('m-d'))
   <article class="alert bg-white shadow-sm fade show align-items-center justify-content-between" style="display: flex;">
      <span class="d-flex align-items-center">
         ...
         <span>
            Happy Birthday <span class="text-primary">{{ auth()->user()->username }}</span>
         </span>
      </span>
...
{{-- marriage greetings --}}
@if (isset(auth()->user()->marriage_date) && auth()->user()->marriage_date->format('m-d') == date('m-d'))
<article class="alert bg-white shadow-sm fade show align-items-center justify-content-between" style="display: flex;">
   <span class="d-flex align-items-center">
      <i class="feather icon-heart-on text-danger fs-4 me-1"></i> 
      <span>
...
@include('widgets.material')

<div class="row">
   <div class="col-xl-6">
      @include('widgets.sale-chart')
   </div>
   <div class="col-xl-6">
      @include('widgets.purchase-chart')
   </div>
</div>
...
@can('viewAny', App\Models\User::class)
  @include('widgets.visit-chart')
  @include('widgets.visit-summary')
@endcan
...
<div class="row">
   <div class="col-xl-6">
     @include('widgets.customer-occasion')
   </div>
   <div class="col-xl-6">
     @include('widgets.employee-occasion')
   </div>
</div>
